...
def split_section_comments(cells):
    """Split code cells containing # ===...=== comment blocks into markdown cells."""
    ...
    for cell in cells:
        ...
        while i < len(lines):
            ...
            if SEP.match(stripped):
                j = i + 1
                inner = []
                while j < len(lines) and not SEP.match(lines[j].strip()):
                ...
                if j < len(lines) and SEP.match(lines[j].strip()):
                    if current:
                        ...
                        current = []
                    segments.append(("markdown", inner))
                    i = j + 1
                    continue
            ...
            if seg_type == "markdown":
                ...
                for ln in seg_lines:
                    ...
                    if s.startswith("# "):
                        ...
                        md_text.append(s)
                result.append(create_markdown_cell("\n".join(md_text) + "\n"))
            else:
                if any(ln.strip() for ln in seg_lines):
                    result.append(
                        create_code_cell("".join(seg_lines).rstrip("\n") + "\n")
                    )
...
def parse_py_to_cells(source_code):
    ...
    3. Convert # ===...=== comment blocks to markdown cells in either case.
    """
    lines = source_code.splitlines(keepends=True)

    # Check if explicit markers exist
    has_explicit_markers = any(re.match(r"^\s*#\s*%%", line.strip()) for line in lines)

    if has_explicit_markers:
        cells = parse_with_markers(lines)
    else:
        cells = split_by_logical_blocks(source_code)

    return split_section_comments(cells)
...
def parse_with_markers(lines):
    ...
    while i < len(lines):
        ...
        markdown_match = re.match(
            r"^\s*#\s*%%\s*\[markdown\]\s*$", stripped, re.IGNORECASE
        )
...
def split_by_logical_blocks(source_code):
    ...
    for i, line in enumerate(lines):
        ...
        if in_definition and current_indent <= definition_indent and stripped and i > 0:
            ...
            if not (
                def_class_pattern.match(stripped) or decorator_pattern.match(stripped)
            ):
                in_definition = False
                if current_block:
